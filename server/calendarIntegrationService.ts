...
export class CalendarIntegrationService
{
  ...
  /**
   * Gerar JWT para autenticação Apple
   */
  private static generateAppleJWT(): string
  {
    try
    {
      ...
      const privateKey = process.env.APPLE_CALENDAR_PRIVATE_KEY;

      if ( !teamId || !keyId || !clientId || !privateKey )
      {
        console.log( '⚠️ Apple Calendar - credenciais não configuradas, usando placeholder' );
        return 'apple_jwt_placeholder';
      }
      ...
      // Decodificar chave privada (geralmente em base64)
      ...
      console.log( '✅ Apple JWT gerado com sucesso' );
      return token;

    } catch ( error )
    {
      console.error( '❌ Erro ao gerar Apple JWT:', error );
      console.log( '⚠️ Usando placeholder para desenvolvimento' );
      return 'apple_jwt_placeholder';
    }
  }
  ...
  ): Promise<any>
  {
    ...
    switch ( provider )
    {
      ...
      case 'apple':
        // Apple usa JWT para autenticação
        tokenUrl = 'https://appleid.apple.com/auth/token';
        const appleJWT = this.generateAppleJWT();
        requestBody = {
          client_id: config.clientId,
  ...
  private static async getCalendarInfo( provider: 'google' | 'apple' | 'outlook', accessToken: string ): Promise<{ id: string, name: string }>
  {
    ...
    switch ( provider )
    {
      ...
      case 'apple':
        // Apple usa CalDAV para acessar calendários
        apiUrl = 'https://caldav.icloud.com/';
        headers[ 'Content-Type' ] = 'application/xml';
        // Para Apple, precisamos fazer uma requisição CalDAV diferente
        return { id: 'primary', name: 'iCloud Calendar' };
    }
