...
export default function TaskResponseForm({ 
  ...
  const validateField = (field: TaskField, value: any): string | null => {
  ...
  const shouldShowField = (field: TaskField): boolean => {
  ...
  const renderField = (field: TaskField) => {
        ...
        fieldElement = (
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
            ...
            <p className="text-sm text-gray-500">
              {value ? `Arquivo: ${value}` : 'Clique para enviar arquivo'}
            </p>
            {!isReadOnly && (
              <input
                type="file"
    ...
    return (
      <div key={field.id} className="space-y-2">
        ...
        {field.validation?.customMessage && !hasError && (
          <p className="text-gray-500 text-xs">{field.validation.customMessage}</p>
        )}
      </div>
    );
  };
  ...
  const visibleFields = template.fields.filter(shouldShowField);
