...
export default function Connectivity() {
  ...
  const testDatabaseMutation = useMutation({
    ...
    onError: (error: any) => {
      setTestResults(prev => ({ ...prev, database: { status: 'error', message: error.message } }));
      toast({
        title: "Erro no Teste",
        description: "Falha ao testar conexão com banco de dados.",
        variant: "destructive",
      });
    },
    onSettled: () => {
  ...
  const testIntegrationMutation = useMutation({
    ...
    onError: (error: any, integrationId) => {
      setTestResults(prev => ({ 
        ...
        integrations: { 
          ...prev.integrations, 
          [integrationId]: { status: 'error', message: error.message } 
        } 
      }));
      toast({
        title: "Erro no Teste",
  ...
  const customTestMutation = useMutation({
    ...
    onSuccess: (data) => {
      setTestResults(prev => ({ ...prev, custom: data }));
      toast({
    ...
    onError: (error: any) => {
      setTestResults(prev => ({ ...prev, custom: { status: 'error', message: error.message } }));
      toast({
        title: "Erro no Teste",
        description: "Falha ao executar teste personalizado.",
        variant: "destructive",
      });
    }
  });
  ...
  const handleCustomTest = () => {
    if (!customTest.url.trim()) {
      toast({
        title: "Erro",
        description: "URL é obrigatória para o teste.",
        variant: "destructive",
      });
      return;
    }
    ...
    const testConfig = {
      ...customTest,
      timeout: parseInt(customTest.timeout),
      headers: customTest.headers ? JSON.parse(customTest.headers) : {},
      body: customTest.body ? JSON.parse(customTest.body) : undefined
    };

    customTestMutation.mutate(testConfig);
  };
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Testes de Conectividade</h1>
            <p className="text-sm text-gray-600 mt-1">
              Verifique a conectividade e teste integrações externas
            </p>
          </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      {getStatusIcon(systemStatus?.server || 'online')}
                      <Badge className={getStatusColor(systemStatus?.server || 'online')}>
            ...
            {(integrations || []).length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Configure integrações na página de Integrações para testá-las aqui
                </p>
              </div>
            )}
          </CardContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={customTest.type} onValueChange={(value) => setCustomTest(prev => ({ ...prev, type: value }))}>
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select value={customTest.method} onValueChange={(value) => setCustomTest(prev => ({ ...prev, method: value }))}>
                    <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCustomTest(prev => ({ ...prev, timeout: e.target.value }))}
                  placeholder="5000"
                />
              </div>
