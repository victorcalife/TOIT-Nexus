...
interface MLConfiguration {
  ...
  kpiAdaptation: {
    ...
    recalculationFrequency: 'daily' | 'weekly' | 'monthly';
...
export default function MLConfiguration() {
  const [config, setConfig] = useState<MLConfiguration>({
    adaptiveEngine: {
      enabled: true,
      sensitivity: 75,
      autoApply: false,
  ...
  const loadMetrics = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          setMetrics({
            accuracy: data.data.mlModels?.behaviorAnalysis?.accuracy || 87.3,
            predictions: data.data.totalPredictions || 1247,
            adaptations: data.data.totalAdaptations || 89,
            lastUpdate: data.data.lastUpdate || new Date().toISOString(),
            status: data.data.overallStatus || 'optimal'
          });
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: "Ciclo de adaptação ML executado com sucesso.",
        });
        await loadMetrics();
      } else {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Configuração ML Adaptativo</h1>
            <p className="text-gray-600">Configure o sistema de Machine Learning e adaptação inteligente</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{metrics.predictions.toLocaleString()}</p>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Sensibilidade de Adaptação: {config.adaptiveEngine.sensitivity}%</Label>
                <Slider
                  value={[config.adaptiveEngine.sensitivity]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({
                      ...prev,
                      adaptiveEngine: { ...prev.adaptiveEngine, sensitivity: value }
                    }))
                  }
                  max={100}
                  min={10}
              ...
              <div className="space-y-2">
                <Label>Taxa de Aprendizado: {config.adaptiveEngine.learningRate}</Label>
                <Slider
                  value={[config.adaptiveEngine.learningRate * 100]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({
                      ...prev,
                      adaptiveEngine: { ...prev.adaptiveEngine, learningRate: value / 100 }
                    }))
                  }
                  max={50}
                  min={1}
        ...
        <TabsContent value="behavior" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Horizonte de Predição: {config.behaviorAnalysis.predictionHorizon} dias</Label>
                <Slider
                  value={[config.behaviorAnalysis.predictionHorizon]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({
                      ...prev,
                      behaviorAnalysis: { ...prev.behaviorAnalysis, predictionHorizon: value }
                    }))
                  }
                  max={90}
                  min={7}
        ...
        <TabsContent value="kpis" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Limite de Confiança: {config.kpiAdaptation.confidenceThreshold}%</Label>
                <Slider
                  value={[config.kpiAdaptation.confidenceThreshold]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({
                      ...prev,
                      kpiAdaptation: { ...prev.kpiAdaptation, confidenceThreshold: value }
                    }))
                  }
                  max={95}
                  min={50}
