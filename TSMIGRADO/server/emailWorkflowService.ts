...
export class EmailWorkflowService
{
  ...
  private async executeWorkflowFromEmailTrigger( trigger: any, email: EmailMessage, extractedData: any ): Promise<void>
  {
    try
    {
      ...
      // Integrar com o workflow engine para execução real
      try
      {
        // Importar o workflow engine dinamicamente para evitar dependências circulares
        const { VisualWorkflowEngine } = await import( './visualWorkflowEngine' );
        ...
        console.log( `🎯 Workflow executado via email trigger: ${ trigger.workflowId }` );

      } catch ( workflowError )
      {
        console.error( '❌ Erro na execução do workflow:', workflowError );
        // Continuar mesmo se o workflow falhar
      }
  ...
  private async fetchImapEmails( account: any ): Promise<EmailMessage[]>
  {
    return new Promise( ( resolve, reject ) =>
    {
      try
      {
        ...
        const imap = new Imap( {
          user: account.email,
          password: account.accessToken ? this.decryptToken( account.accessToken ) : '',
        ...
        imap.once( 'ready', () =>
        {
          console.log( `📬 Conectado ao IMAP: ${ account.email }` );
          ...
          imap.openBox( 'INBOX', true, ( err: any, box: any ) =>
          {
            ...
            imap.search( [ 'UNSEEN', [ 'SINCE', since ] ], ( err: any, results: any ) =>
            {
              if ( err )
              {
                console.error( '❌ Erro na busca IMAP:', err );
                return reject( err );
              }
              ...
              console.log( `📬 Encontrados ${ results.length } emails novos` );

              const fetch = imap.fetch( results.slice( 0, 50 ), { // Limitar a 50 emails
                bodies: '',
                struct: true
              } );
              ...
              {
                console.error( '❌ Erro no fetch IMAP:', err );
                reject( err );
              } );

              fetch.once( 'end', () =>
              {
                console.log( `✅ Processados ${ emails.length } emails via IMAP` );
                imap.end();
                resolve( emails );
              } );
            } );
      ...
      {
        console.error( '❌ Erro geral IMAP:', error );
        reject( error );
      }
    } );
  }
