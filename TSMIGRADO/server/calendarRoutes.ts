...
/**
 * GET /api/calendar/test-integration
 * Testar configuração das integrações de calendário
 */
router.get( '/test-integration', authMiddleware, async ( req, res ) =>
{
  try
  {
    console.log( '🧪 Testando configuração de integrações de calendário...' );

    const testResults = {
      google: {
        configured: !!( process.env.GOOGLE_CALENDAR_CLIENT_ID && process.env.GOOGLE_CALENDAR_CLIENT_SECRET ),
        clientId: process.env.GOOGLE_CALENDAR_CLIENT_ID ? 'Configurado' : 'Não configurado',
        status: 'ready'
      },
      outlook: {
        configured: !!( process.env.MICROSOFT_CALENDAR_CLIENT_ID && process.env.MICROSOFT_CALENDAR_CLIENT_SECRET ),
        clientId: process.env.MICROSOFT_CALENDAR_CLIENT_ID ? 'Configurado' : 'Não configurado',
        status: 'ready'
      },
      apple: {
        configured: !!( process.env.APPLE_CALENDAR_CLIENT_ID && process.env.APPLE_CALENDAR_TEAM_ID && process.env.APPLE_CALENDAR_KEY_ID ),
        clientId: process.env.APPLE_CALENDAR_CLIENT_ID ? 'Configurado' : 'Não configurado',
        jwtGeneration: 'functional',
        status: 'ready'
      }
    };

    const overallStatus = Object.values( testResults ).every( provider => provider.configured ) ? 'all_ready' : 'partial_ready';
