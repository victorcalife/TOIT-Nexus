...
const FIELD_TYPES = [
  ...
  { value: 'time', label: 'Hora', icon: Calendar },
  { value: 'datetime', label: 'Data e Hora', icon: Calendar },
  { value: 'email', label: 'Email', icon: Mail },
  { value: 'url', label: 'URL', icon: Mail },
  { value: 'checkbox', label: 'Checkbox', icon: CheckSquare },
  { value: 'radio', label: 'Múltipla Escolha (Única)', icon: CheckSquare },
...
export default function TaskFieldBuilder({ fields, onChange, preview = false }: TaskFieldBuilderProps) {
  ...
  const addField = (type: TaskField['type']) => {
    const newField: TaskField = {
      ...
      required: false,
      options: ['radio', 'select', 'multiselect'].includes(type) ? ['Opção 1', 'Opção 2'] : undefined
    };
  ...
  const updateFieldOptions = (fieldId: string, options: string[]) => {
    updateField(fieldId, { options: options.filter(o => o.trim() !== '') });
  };

  const renderFieldIcon = (type: TaskField['type']) => {
  ...
  const renderFieldPreview = (field: TaskField) => {
    const commonProps = {
